...
This policy covers `protowire-csharp` — the C# port of the `protowire`
...
- Schema-validation bypasses that let invalid messages reach
  application code.
- Any `unsafe`-block soundness issue in `src/Protowire.Sbe/View.cs` or
  similar zero-allocation paths.
...
- Issues in `Google.Protobuf` itself — file those upstream at
...
on every PR via the `cmd/Protowire.CheckDecode` harness. A regression
on the corpus blocks merge.
